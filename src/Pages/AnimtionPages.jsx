...
export default function AnimationPages() {

  
  const [numberPage, setNumberPage] = useState(1);

  const { data: animationP, isLoading, isError, error } = useQuery(
    ["animationPage", numberPage],
    async () => {
      const response = await axios.get(
        `https://api.themoviedb.org/3/discover/movie?api_key=d52f00854ed915b688d5abec54bce588&with_genres=16&language=en-US&page=${numberPage}&sort_by=popularity.desc`
      );
    ...
    <div className="w-full h-[245vh] bg-[#0f0e0e] text-white pt-20 pl-8 pr-8 pb-20">
      <div className="grid grid-cols-5 gap-5">
        ...
        <div className="text-center w-[1840px] h-full">
          ...
          <button
            className="text-white"
            disabled={numberPage === animationP?.total_pages || isLoading}
            onClick={() => setNumberPage((prev) => prev + 1)}
          >
            Next
          </button>
